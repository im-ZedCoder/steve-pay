...
const SMS_CURL = `${K('curl')} -X POST ${S('https://steve-pay.ir/sms')} \\
  -H ${S('"X-API-Key: sk_live_<key-id><secret>"')} \\
  -H ${S('"Content-Type: application/json"')} \\
  -d ${P("'")}{
...
const NODE_FLOW = `${K('const')} BASE = ${S("'https://steve-pay.ir/api/v1'")};
...
export function docsPage(): string {
  const baseUrlNote = docsNote(
    `<b>آدرس پایه:</b> ${code('https://steve-pay.ir')} — همه پاسخ‌ها JSON هستند و هر پاسخ، موفق یا ناموفق، هدر ${code('X-Request-Id')} دارد. اگر خطایی دیدید که خودتان حل نکردید، همین شناسه را برای پشتیبانی بفرستید: با آن، لاگ و رکورد ممیزی همان درخواست پیدا می‌شود.`,
  );
...
${table(
  ...
  [
    ...
    [code('Idempotency-Key'), 'برای پرداخت توصیه‌شده', 'هر رشت یکتا با حداکثر ۲۵۵ نویسه. See <a href="#idempotency">Idempotency</a>.'],
...
${codeBlock({
  ...
  ${P('"sms"')}: {
    ...
    ${P('"webhookUrl"')}: ${P('"https://steve-pay.ir/sms"')}
  },
...
${codeBlock({ label: 'CURL', note: 'آزمایش دستی', html: SMS_CURL })}

<h3>اتصال برنامه فورواردر (اندروید)</h3>
<p>روی اندرویدی که پیامک بانک را می‌گیرد، برنامه‌ای لازم است که پیامک را هنگام رسیدن به یک آدرس بفرستد. هر برنامه‌ای که بتواند درخواست HTTP با هدر دلخواه بفرستد کار می‌کند؛ الگوی تنظیم این است:</p>
${table(
  ['تنظیم', 'مقدار'],
  [
    ['آدرس', code('https://steve-pay.ir/sms')],
    ['روش', code('POST')],
    ['هدر ۱', code('X-API-Key: sk_live_<key-id><secret>')],
    ['هدر ۲', code('Content-Type: application/json')],
    ['بدنه', 'الگوی JSON زیر، با متغیرهای خود برنامه برای متن و فرستنده'],
    ['فرستنده‌های مجاز', 'فقط شماره‌های بانکی. فوروارد کردن همه پیامک‌ها رمزهای یک‌بارمصرف بانک را هم به سرور می‌فرستد.'],
  ],
)}
${codeBlock({ label: 'BODY', note: 'الگوی برنامه فورواردر', html: FORWARDER_BODY })}
${docsNote(
  '<b>فقط برنامه فورواردر را روی حساب خودتان تنظیم کنید.</b> متن پیامک بانک ممکن است موجودی حساب شما را داشته باشد. سامانه پیامک‌های بی‌ربط را جایی نمایش نمی‌دهد، اما همان‌قدر که به ما اعتماد دارید، بهتر است فرستنده را هم محدود کنید.',
)}

<h3>تست اتصال</h3>
<p>در <a href="/dashboard/sms">پنل پذیرنده → پیامک</a> دکمه «تست اتصال» یک پیامک نمونه با یک توکن یک‌ساعته نشان می‌دهد. آن را در برنامه فورواردر به‌جای پیامک بانک بگذارید و بفرستید؛ اگر رسید، مسیر درست است و از این پس پیامک‌های واقعی هم می‌رسند.</p>
...
${codeBlock({ label: 'NODE.JS', note: 'ساخت پرداخت', html: NODE_FLOW })}
${codeBlock({ label: 'CURL', note: 'کل مسیر در یک خط', html: CURL })}
${table(
  ['گام', 'کار سمت شما'],
  [
  ...
  return docsShell(
    {
      ...
      nav: [
        ...
        {
          group: 'پیامک بانک',
          links: [{ href: '#sms', label: 'فورواردر و تطبیق' }],
        },
        {
          group: 'وبهوک',
